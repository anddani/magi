...
fn test_select_confirm_rebase_elsewhere_context_returns_rebase_message() {
    ...
    let mut commits = get_log_entries(&repo, &LogType::Current, true, false, true).unwrap();
...
// ── Remove commit — popup key and selection ───────────────────────────────────

#[test]
fn test_k_in_rebase_popup_shows_remove_commit_select() {
    let test_repo = TestRepo::new();
    let mut model = create_model_from_test_repo(&test_repo);
    model.popup = Some(rebase_popup(false));
    ...
    assert_eq!(
        result,
        Some(Message::ShowCommitSelect(CommitSelect::RemoveCommit))
    );
}

#[test]
fn test_k_key_has_no_effect_in_in_progress_popup() {
    let test_repo = TestRepo::new();
    let mut model = create_model_from_test_repo(&test_repo);
    model.popup = Some(rebase_popup(true));
...
#[test]
fn test_remove_commit_on_commit_shows_confirmation() {
    let test_repo = TestRepo::new();
    test_repo
        .commit_file("file1.txt", "content1", "First commit")
        .commit_file("file2.txt", "content2", "Second commit");
    ...
    assert!(matches!(
        &state.on_confirm,
        ConfirmAction::RemoveCommit(hash) if *hash == expected_hash
    ));
}

#[test]
fn test_remove_commit_confirmation_routes_to_rebase_command() {
    let test_repo = TestRepo::new();
    let mut model = create_model_from_test_repo(&test_repo);
    model.popup = Some(PopupContent::Confirm(
        magi::model::popup::ConfirmPopupState {
            message: "Remove commit abc1234?".to_string(),
            on_confirm: ConfirmAction::RemoveCommit("abc1234".to_string()),
        },
    ));
    ...
    assert_eq!(
        result,
        Some(Message::Rebase(RebaseCommand::RemoveCommit(
            "abc1234".to_string()
        )))
    );
}
...
#[test]
fn test_remove_commit_not_on_commit_shows_current_log_pick_view() {
    let test_repo = TestRepo::new();
    test_repo.commit_file("file1.txt", "content1", "First commit");
    ...
    assert_eq!(model.log_pick_on_select, Some(OnSelect::RemoveCommit));
}

#[test]
fn test_select_confirm_remove_commit_returns_rebase_message() {
    let test_repo = TestRepo::new();
    test_repo.commit_file("file1.txt", "content1", "First commit");
    ...
    assert_eq!(
        result,
        Some(Message::Rebase(RebaseCommand::RemoveCommit(expected_hash)))
    );
    assert_eq!(model.view_mode, ViewMode::Status);
    assert!(model.log_pick_on_select.is_none());
}
...
#[test]
fn test_remove_commit_update_removes_commit() {
    let test_repo = TestRepo::new();
    test_repo.commit_file("file1.txt", "content1", "First commit");
    let target = test_repo.head_hash();
    test_repo.commit_file("file2.txt", "content2", "Second commit");
    ...
    update(
        &mut model,
        Message::Rebase(RebaseCommand::RemoveCommit(target)),
    );

    // The rebase runs to completion; the commit and its changes are gone
    assert!(!rebase_in_progress(test_repo.repo_path()));
    assert!(model.toast.is_some(), "Expected a toast after remove");
