use magi::{
    ...
    msg::{FixupType, LogType, Message, ShowSelectDialog, update::update},
};
...
fn test_show_fixup_commit_select_without_staged_changes_shows_toast() {
    ...
    let result = update(
        &mut model,
        Message::ShowSelect(ShowSelectDialog::FixupCommit(FixupType::Fixup)),
    );
...
fn test_show_squash_commit_select_without_staged_changes_shows_toast() {
    ...
    let result = update(
        &mut model,
        Message::ShowSelect(ShowSelectDialog::FixupCommit(FixupType::Squash)),
    );
