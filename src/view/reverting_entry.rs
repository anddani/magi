...
/// Render a single line in the "Reverting" sequencer section.
///
/// - `is_current = true`  → the commit currently stopped on (shows "stop" in deletion color)
/// - `is_current = false` → a pending commit in the sequence (shows "revert" in normal text color)
pub fn get_lines(
    hash: &str,
    message: &str,
    is_current: bool,
    ...
    // Label: "stop" for the current stopped commit, "revert" for pending
    let (label, label_style) = if is_current {
        ("stop ", Style::default().fg(theme.diff_deletion))
    } else {
        ("revert ", Style::default().fg(theme.text))
    };
...
mod tests {
    ...
    fn test_current_entry_shows_stop_label() {
        ...
        assert!(!texts.iter().any(|s| s.contains("revert")));
    }

    #[test]
    fn test_pending_entry_shows_revert_label() {
        let theme = test_theme();
